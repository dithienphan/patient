    ...
    stages {
        ...
        stage('Init') {
            ...
            steps {
                ...
                git url: 'https://github.com/dithienphan/patient', branch: 'main', poll: true
            }
        }
        ...
        stage('Test, Codeanalyse, Security, Dok.') {
            ...
            post {
                ...
                success {
                    script {
                        if (fileExists("${env.WORKSPACE}/patient.zip")) {
                            sh 'rm patient.zip'
                        }
                    }
                    // https://www.jenkins.io/doc/pipeline/steps/pipeline-utility-steps/#zip-create-zip-file
                    zip zipFile: 'patient.zip', archive: false, dir: 'dist'
                    // jobs/patient/builds/.../archive/patient.zip
                    archiveArtifacts 'patient.zip'
                }
            }
        }
